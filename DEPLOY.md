...
## 4b. Sync manual por volcado (HISTÓRICO — ya no usar)

> ⚠️ **CUTOVER HECHO: no re-importar el dump.** MySQL es ahora la **fuente única de verdad**
> y la app web escribe directamente. El volcado hace **DROP + CREATE + recarga** desde SQL
> Anywhere, así que re-importarlo **borraría** todo lo cargado/editado en la web. Esta sección
> queda solo como referencia histórica de cómo funcionaba la sync durante la transición.

Cómo funcionaba (transición): el MySQL de Easypanel es **interno** (no expuesto), así que la
actualización se hacía por volcado:
...
## 5. Cutover (COMPLETADO)

El cutover **ya se realizó**: se dejó de usar el PowerBuilder y se detuvo la sync. El MySQL de
la web es la **única fuente de verdad** y la app escribe directamente ahí. No re-importar más
el dump (ver aviso en 4b).
